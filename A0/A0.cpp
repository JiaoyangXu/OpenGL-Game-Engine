...
const float shape_change_value = 1.0f;
...
void A0::guiLogic()
{
	...
	ImGui::Begin("Shape Properties", &showDebugWindow, ImVec2(100,100), opacity,
		...
		// Control the rotation angle of the triangle
...
bool A0::keyInputEvent(int key, int action, int mods) {
	...
	if (action == GLFW_PRESS) {
		if (key == GLFW_KEY_EQUAL) {
			...
			m_shape_size += m_shape_size < 9.0 ? shape_change_value : 0.0f;
			eventHandled = true;
		}
		if (key == GLFW_KEY_MINUS) {
			cout << "- key pressed" << endl;
			m_shape_size -= m_shape_size > 1.0 ? shape_change_value : 0.0f;
			eventHandled = true;
		}
		if (key == GLFW_KEY_Q) {
			glfwSetWindowShouldClose(m_window, GL_TRUE);
...
//----------------------------------------------------------------------------------------
/*
 * Reset all properties about the triangle
 */
void A0::reset_triangle() {
	m_shape_rotation = 0.0f;
	m_shape_size = 1.0f;
	m_shape_translation = vec2(0.0f);
	m_shape_color = glm::vec3(1.0f, 1.0f, 1.0f);
